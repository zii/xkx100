...
int skill_rating(object sk) { return (60 - sk->learn_bonus() - sk->practice_bonus()) / 6; }
int is_category(string arg) { return member_array(arg, special_cats) >= 0 || !undefinedp(valid_type[arg]); }
...
// 按视觉宽度补齐空格（strwidth 是 FluffOS 内置 efun）
string pv(string s, int w) {
    while (strwidth(s) < w) s += " ";
    return s;
}
...
int show_categories(object me) {
    ...
    // ─ 占 2 列宽，框宽 = 2 + 30*2 + 2 = 64，内容区 60vw
    string msg = "\n" HIC "┌──────────────────────────────┐\n" NOR;
...
int list_skills(object me, string cat) {
    ...
    string *sorted = sort_array(keys(data), (: sort_skill :));
    ...
    for (int i = 0; i < sizeof(sorted); i++) {
        ...
        string rc = ra >= 9 ? HIR : ra >= 7 ? HIM : ra >= 5 ? HIY : ra >= 3 ? HIG : "";

        string inner = "";
        inner += pv(sprintf("%d", i + 1), 2) + "  ";
        inner += rc + pv(chinese_number(ra), 2) + NOR + "  ";
        inner += pv(nm, 24) + "  ";
        inner += pv(id, 20);

        msg += HIC "│" NOR " " + inner + pv("", 58 - strwidth(inner)) + HIC " │\n" NOR;
    }
    msg += HIC "└──────────────────────────────┘\n" NOR;
    me->start_more(msg);
    return 1;
